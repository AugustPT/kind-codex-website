...
const ANGLES = `ANGLE — every post uses a HIGH-PERFORMING angle (set the "angle" field). It is ALWAYS about the READER and their business, never about us. DEFAULT most of the batch (3 of 5) to "almost-lost story" — the quiet micro-story is what stops the scroll and the look August wants MORE of. Rotate the others in as variety, not the majority. Choose from:
...
- "contrarian": a defensible against-the-grain take. e.g. "You don't need more customers. You need to stop losing the ones already reaching out."
