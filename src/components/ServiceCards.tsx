...
// The six abstract diagrams, kept by position so any card set reuses the same
// visual language as the homepage. Card TEXT is swapped per page; art stays.
const DIAGRAMS: React.ReactNode[] = [
  // 0 — Layout clarity
  <svg key="d0" viewBox="0 0 200 100" className="w-full max-w-[160px] h-auto overflow-visible">
    ...
    <circle cx="18" cy="16" r="2" fill="#e7e5e4" /><circle cx="24" cy="16" r="2" fill="#e7e5e4" /><circle cx="30" cy="16" r="2" fill="#e7e5e4" />
    <line x1="150" y1="16" x2="175" y2="16" stroke="#e7e5e4" strokeWidth="2" />
    <line x1="30" y1="40" x2="110" y2="40" stroke="#1c1917" strokeWidth="3.5" strokeLinecap="round" />
    <line x1="30" y1="50" x2="90" y2="50" stroke="#c2410c" strokeWidth="3.5" strokeLinecap="round" />
    <line x1="30" y1="65" x2="170" y2="65" stroke="#e7e5e4" strokeWidth="1.5" />
    <line x1="30" y1="72" x2="140" y2="72" stroke="#e7e5e4" strokeWidth="1.5" />
    <motion.rect x="145" y="34" width="30" height="12" rx="2" className="fill-[#c2410c]" animate={{ opacity: [0.7, 1, 0.7] }} transition={{ duration: 2, repeat: Infinity }} />
  </svg>,
  // 1 — Timeline / pre-qualification
  <svg key="d1" viewBox="0 0 200 100" className="w-full max-w-[160px] h-auto overflow-visible">
    ...
    <path d="M 68,50 L 132,50" fill="none" stroke="rgba(194, 65, 12, 0.2)" strokeWidth="1.5" strokeDasharray="4 3" />
    ...
    <path d="M 152,44 C 152,42 162,42 162,44 L 164,48 C 164,49 162,51 157,51 C 152,51 150,49 148,48 L 152,44 Z" fill="none" stroke="#1c1917" strokeWidth="1.5" />
    <circle cx="162" cy="56" r="3" fill="#1c1917" />
  </svg>,
  // 2 — Radial channels
  <svg key="d2" viewBox="0 0 200 100" className="w-full max-w-[160px] h-auto overflow-visible">
  ...
  // 3 — Graph / tracking
  <svg key="d3" viewBox="0 0 200 100" className="w-full max-w-[160px] h-auto overflow-visible">
    ...
    <motion.path d="M 20,70 Q 60,65 100,45 T 180,25" fill="none" stroke="#c2410c" strokeWidth="2.5" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1.5 }} />
    <circle cx="100" cy="45" r="3" fill="#1c1917" /><circle cx="180" cy="25" r="3" fill="#1c1917" />
  </svg>,
  // 4 — Engagement loop
  <svg key="d4" viewBox="0 0 200 100" className="w-full max-w-[160px] h-auto overflow-visible">
    ...
    <path d="M 125,50 C 160,50 160,85 100,85 C 40,85 40,50 75,50" fill="none" stroke="#c2410c" strokeWidth="2" strokeDasharray="4 3" />
    <path d="M 70,46 L 76,50 L 70,54" fill="none" stroke="#c2410c" strokeWidth="2" strokeLinecap="round" />
    <circle cx="140" cy="50" r="3.5" fill="#e7e5e4" /><circle cx="60" cy="50" r="3.5" fill="#e7e5e4" />
  </svg>,
  // 5 — Tiered progression
  <svg key="d5" viewBox="0 0 200 100" className="w-full max-w-[160px] h-auto overflow-visible">
...
const DEFAULT_EYEBROW = "Our Focus";
const DEFAULT_HEADING = "What KindCodex improves";
const DEFAULT_CARDS: ServiceCard[] = [
  { title: "Better Website", description: "Customers instantly understand what you do, who you help, and why they should contact you." },
  { title: "Better Calls", description: "The right questions are answered before people reach out, so you get better conversations." },
  { title: "Better Visibility", description: "Your Google Business Profile, photos, reviews, website, and message work together." },
  { title: "Better Tracking", description: "See where people came from, what they clicked, and what made them call, message, or leave." },
  { title: "Better Follow-Up", description: "Bring back people who looked but did not buy the first time." },
  { title: "Better Sales Path", description: "Help new and existing customers choose the right next step." },
];
          ...
          {cards.slice(0, 6).map((card, idx) => (
            ...
            >
              <div className="w-full h-32 flex items-center justify-center bg-stone-50/60 border border-stone-150 rounded-lg mb-6 overflow-hidden relative">
                {DIAGRAMS[idx % DIAGRAMS.length]}
              </div>
              <div className="flex flex-col gap-2">
                <h3 className="text-lg font-bold text-stone-900 tracking-tight">{card.title}</h3>
                <p className="text-sm font-semibold text-stone-500 leading-relaxed">{card.description}</p>
              </div>
            </motion.div>
          ))}
        </motion.div>
